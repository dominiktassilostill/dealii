 ...
 * ------------------------------------------------------------------------
 *
 * Test function GridGenerator::convert_simplex_to_hypercube_mesh() in 2D
...
{
  GridGenerator::subdivided_hyper_cube_with_simplices(triangulation, 4);
}
...
{
  Triangulation<dim, spacedim> temp;
  GridGenerator::quarter_hyper_ball(temp);
...
main()
{
  ...
  // dim = spacedim = 2
  deallog.push(
    "2D: conversion triangulation with tri elements to quad elements: ");
  check_file<2, 2>();
  deallog.pop();
  ...
  // dim = spacedim = 3
  deallog.push(
    "3D: conversion triangulation with hex elements to tet elements: ");
  check_file<3, 3>();
  deallog.pop();
  ...
  // dim = spacedim = 3
  deallog.push(
    "3D: conversion triangulation with hex elements to tet elements + refinement: ");
  check_file<3, 3>(1);
  deallog.pop();
}
