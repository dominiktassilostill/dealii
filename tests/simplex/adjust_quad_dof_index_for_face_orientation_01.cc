      ...
      for (unsigned int r = 0; r < 24; ++r)
        {
          ...
          deallog << "Face number, orientation: " << f << " "
                  ...
                  << std::endl;
          deallog << "Neighbor face number, orientation: " << face_no << " "
          ...
          // for(const auto i: neighbor_dof_indices)
          //   deallog << i << " ";
          // deallog << std::endl;

          // deallog << "DoFs per quad: " << fe.n_dofs_per_quad(f) << std::endl;
          std::vector<types::global_dof_index> dof_indices_on_quad(
            fe.n_dofs_per_quad(f), -1);
          std::vector<types::global_dof_index> neighbor_dof_indices_on_quad(
            fe.n_dofs_per_quad(face_no), -1);

          // cell->face(f)->get_dof_indices(dof_indices_on_quad);
          ...
          // neighbor_cell->face(face_no)->get_dof_indices(neighbor_dof_indices_on_quad);
